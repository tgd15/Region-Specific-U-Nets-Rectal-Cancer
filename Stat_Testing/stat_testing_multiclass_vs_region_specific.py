...
def parse_path(expertpath, segpath):
    ...
    if(ex_unet == seg_unet and ex_metric == seg_metric):
        ...
        expert = seg_expert
        metric = ex_metric
    elif("Multiclass" in ex_unet and ex_metric == seg_metric):
        unet = seg_unet
        expert = seg_expert
        metric = ex_metric
    else:
        print(ex_unet + "," + ex_metric)
        print(seg_unet + "," + seg_metric + "," + seg_expert)
...
def gen_boxplot(multiclass_np, region_specific_np, unet, expert, metric, box_path, out_dpi=300, appendString = None, multiclass = False):
    ...
    if(appendString is not None):
        title = "Multiclass U-Net vs. Region-Specific U-Net: "  + unet + " for " + expert + "\n" + metric + appendString
    ...
    # Create figure
    fig, ax = plt.subplots()
    ax.boxplot([multiclass_np, region_specific_np], labels=["Multiclass" , "Region-Specific"], boxprops=boxprops, medianprops=medianprops, whiskerprops=whiskerprops)
    ax.set_title(title)
...
outpath = '/Volumes/GoogleDrive/My Drive/tom/Rectal Segmentation/Data-MultipleExperts/Figures/Stat_Testing/2022_11_01_Results/'
...
output_table = pd.DataFrame(columns=["Region Specific U-Net", "Compared To", "Expert", "Metric", "p-Value", "Statistic", "Significance", "Confidence"])

# Open document with all fielapths
filepaths = pd.read_excel("/Volumes/GoogleDrive/My Drive/tom/Rectal Segmentation/Data-MultipleExperts/Figures/Stat_Testing/2022_11_01_Stat_Filepaths.xlsx")
...
region_specific_paths = filepaths["region-specific"].to_list()
region_specific_paths = [s.replace("\ufeff","") for s in region_specific_paths]
...
for index, path in enumerate(multiclass_paths):
    ...
    region_specific_excel_path = region_specific_paths[index]

    # Parse path for unet, expert number, and metric
    unet, expert, metric = parse_path(multiclass_excel_path, region_specific_excel_path)
    
    print("U-Net: %s \n Expert: %s \n Metric: %s \n" % (unet, expert, metric))

    # Load in the metrics
    multiclass_df, multiclass_np = load_excel(multiclass_excel_path, metric)
    region_specific_df, region_specific_np = load_excel(region_specific_excel_path, metric)

    if(append is None):
        # Generate boxplot
        gen_boxplot(multiclass_np, region_specific_np, unet, expert, metric, boxplot_out, multiclass = True)
    ...
    if(append is not None and "Percentile" in append):
        ...
        region_specific_np = calculate_percentile(region_specific_np, percentile)
        gen_boxplot(multiclass_np, region_specific_np, unet, expert, metric, boxplot_out, appendString=append)
        
    if(append == "_IQR"):
        # Calculate IQR and generate boxplots
        multiclass_np, expert_IQR = calculate_IQR(multiclass_np)
        region_specific_np, expert_IQR = calculate_IQR(region_specific_np)
        gen_boxplot(multiclass_np, region_specific_np, unet, expert, metric, boxplot_out, appendString=append)
    
    # Compute p-value via Wilcoxon Ranksum
    statistic, pval = ranksums(multiclass_np, region_specific_np)
    
    # Compute p-value via t-test
    # statistic, pval = ttest_ind(multiclass_np, region_specific_np)
    
    # Calculate confidence interval
    interval = st.t.interval(alpha=0.95, df=len(region_specific_np)-1, loc=np.mean(region_specific_np), scale=st.sem(region_specific_np))
    ...
    temp_df = {'Region Specific U-Net': unet, 'Compared To': "Multiclass", "Expert": expert, 'Metric': metric, 'p-Value': pval, 'Statistic':statistic, "Significance":significance, "Confidence": interval}
