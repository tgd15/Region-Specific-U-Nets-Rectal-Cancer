...
def parse_path(multiclass_path, expertpath):
    ...
    ----------
    multiclass_path : str
        Path to excel document containing metrics multiclass U-Net
    expertpath : str
        Path to excel document containing metrics expert vs. expert agreement (i.e., inter-reader agreement)

    Raises
    ------
    ValueError
        Raise error when multiclass_path and expertpath do not have the same U-Net and metric in filename.
    ...
    def do_parse(path, multiclass = False):
        ...
        path : str
            Filename to parse.
        multiclass : bool
            Indicate whether input filepaths are for multiclass U-Net
        ...
        if(multiclass):
            ...
            index_after_region = [idx for idx, s in enumerate(thepath.parts) if 'CCA' in s][0]
            region_index_pos = index_after_region - 1
            
            unet = unet + " " + thepath.parts[region_index_pos]
    
            expert = thepath.parts[index_after_region]
    ...
    multiclass_unet, multiclass_expert, multiclass_metric = do_parse(multiclass_path, multiclass = True)
    expert_region, expert_to_expert, expert_metric = do_parse(expertpath)
    
    if(multiclass_unet == expert_region and multiclass_metric == expert_metric):
        unet = multiclass_unet
        expert = expert_to_expert
        metric = multiclass_metric
    elif(multiclass_unet == "Rectal Wall U-Net" and expert_region == "Segmentation Fusion" and multiclass_metric == expert_metric):
        unet = expert_region
        expert = expert_to_expert
        metric = multiclass_metric
    elif("Multiclass" in multiclass_unet and multiclass_metric == expert_metric):
        unet = multiclass_unet
        expert = multiclass_expert
        metric = multiclass_metric
    else:
        print(multiclass_unet + "," + multiclass_metric)
        print(expert_region + "," + expert_metric + "," + expert_to_expert)
...
def load_excel(excel_path, metric):
    ...
    try:
        ...
        print("Median value not included in spreadsheet")
...
def gen_boxplot(multiclass_np, expert_np, unet, expert, metric, box_path, out_dpi=300, appendString = None, multiclass = False):
    ...
    if(appendString is not None):
        title = "Multiclass U-Net vs. Inter-reader Agreement: "  + unet + " for " + expert + "\n" + metric + appendString
        filename = box_path + unet + '_' + metric + '_' + expert + appendString + '.png'
    else:
        title = "Multiclass U-Net vs. Inter-reader Agreement: "  + unet + " for " + expert + "\n" + metric
        filename = box_path + "Multiclass_" + unet + '_' + metric + '_' + expert + '.png'
    
    # Create figure
    fig, ax = plt.subplots()
    ax.boxplot([multiclass_np, expert_np], labels=["Multiclass" , "Expert"], boxprops=boxprops, medianprops=medianprops, whiskerprops=whiskerprops)
    ax.set_title(title)
...
outpath = '/Volumes/GoogleDrive/My Drive/tom/Rectal Segmentation/Data-MultipleExperts/Figures/Stat_Testing/2022_12_06_Results/'
...
output_table = pd.DataFrame(columns=["U-Net", "Compared To", "Expert", "Metric", "p-Value", "Statistic", "Significance", "Confidence"])

# Open document with all fielapths
filepaths = pd.read_excel("2022_12_01_Stat_Filepaths_multiclass_vs_experts.xlsx")

# Create lists of expert and prediction filepaths
multiclass_paths = filepaths["multiclass"].to_list()
multiclass_paths = [s.replace("\ufeff","") for s in multiclass_paths]
...
for index, path in enumerate(multiclass_paths):
    multiclass_excel_path = path
    expert_excel_path = expert_paths[index]

    # Parse path for unet, expert number, and metric
    unet, expert, metric = parse_path(multiclass_excel_path, expert_excel_path)
    
    print("U-Net: %s \n Expert: %s \n Metric: %s \n" % (unet, expert, metric))

    # Load in the metrics
    multiclass_df, multiclass_np = load_excel(multiclass_excel_path, metric)
    expert_df, expert_np = load_excel(expert_excel_path, metric)

    if(append is None):
        # Generate boxplot
        gen_boxplot(multiclass_np, expert_np, unet, expert, metric, boxplot_out, multiclass = True)
        
    if(append is not None and "Percentile" in append):
        # Calculate 90th percentile and generate boxplots
        multiclass_np = calculate_percentile(multiclass_np, percentile)
        expert_np = calculate_percentile(expert_np, percentile)
        gen_boxplot(multiclass_np, expert_np, unet, expert, metric, boxplot_out, appendString=append)
        
    if(append == "_IQR"):
        # Calculate IQR and generate boxplots
        multiclass_np, expert_IQR = calculate_IQR(multiclass_np)
        expert_np, expert_IQR = calculate_IQR(expert_np)
        gen_boxplot(multiclass_np, expert_np, unet, expert, metric, boxplot_out, appendString=append)
    
    # Compute p-value via Wilcoxon Ranksum
    statistic, pval = ranksums(multiclass_np, expert_np)
    
    # Compute p-value via t-test
    # statistic, pval = ttest_ind(multiclass_np, expert_np)
    
    # Calculate confidence interval
    interval = st.t.interval(alpha=0.95, df=len(expert_np)-1, loc=np.mean(expert_np), scale=st.sem(expert_np))
    ...
    temp_df = {'U-Net': unet, 'Compared To': "Expert", "Expert": expert, 'Metric': metric, 'p-Value': pval, 'Statistic':statistic, "Significance":significance, "Confidence": interval}
